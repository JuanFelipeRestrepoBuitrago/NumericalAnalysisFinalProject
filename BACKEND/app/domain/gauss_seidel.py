...
class GaussSeidel:
            ...
            for i in range(A.shape[0]):
                ...
                for j in range(A.shape[1]):
                    ...
                    if i != j:
                        ...
                        if x_current.shape[0] == 1:
                            sum_row += (A[i, j] * x_new[0, j]).evalf(self.precision)
                        elif x_current.shape[1] == 1:
                            sum_row += (A[i, j] * x_new[j, 0]).evalf(self.precision)
                ...
                if x_new.shape[0] == 1:
                    x_new[0, i] = ((b_element - sum_row).evalf(self.precision) / A[i, i]).evalf(self.precision)
                elif x_new.shape[1] == 1:
                    x_new[i, 0] = ((b_element - sum_row).evalf(self.precision) / A[i, i]).evalf(self.precision)
    ...
    def matrix_solve(self, tol: float, max_iter: int = 100, A: sp.Matrix = None, b: sp.Matrix = None, x_initial: sp.Matrix = None, absolute_error: bool = True, order: int = 0) -> Tuple[List[int], List[List[str]], List[str], str]:
        ...
        while error > tol and counter < max_iter:
            ...
            T = ((D-L).evalf(self.precision).inv().evalf(self.precision) * U).evalf(self.precision)
            C = ((D-L).evalf(self.precision).inv().evalf(self.precision) * b_element).evalf(self.precision)
