...
function calculateSORMethod() {
   ...
   const relaxationFactor = parseFloat(document.getElementById('relaxation_factor').value);

   if (matrix.some(row => row.includes(NaN)) || vectorB.includes(NaN) || xInitial.includes(NaN)) {
      alert("Por favor, rellena todos los campos de la matriz, el vector, el vector inicial y el factor de relajación.");
      return;
   }
